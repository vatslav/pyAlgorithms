...
def Pj(j,k,t=table): #теперь нафик не надо их передовать, тем более что при запуске с аргументами есть ошибка
    print('t=',t)
    k = len(t)
    j = len(t[0])
    for x in t:
        if len(x)!=k:
            print('tatata')
            showerror('Ошиибка',"Матрица должна быть квадратной")
            return
    ...
    for x in range(1,k): #для строк с 1 до к
        ...
        for J in range(j): # в каждой строки (j штук)
            print("подцикл")
            P[x+1].append( sum( [pi(x,i) * t[i][J] for i in range(len(t))] ) )#умножение строки на столбец и суммирование, вродь :)
    print("ДОСТИГНУТ КОНЕЦ PJ")
    return P
def myround(x):
    c = str (x)
    position = c.find('.')
